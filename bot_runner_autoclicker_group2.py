# ==============================================================================
# SHIBA INU AUTO-TAP — نسخه نهایی با استریک و اسپین دو مرحله‌ای و ضداسپم
# ==============================================================================
...
BASE_URL = "https://shibabotrunnerautoclickgroup2.alishiba4.workers.dev/"
...
def get_account_headers(acc):
    ...
    headers = {
        ...
        "X-Worker-Auth": "MySecretPass_987654"  # دقیقا همان رمزی که داخل هر دو ورکر گذاشتی
    }
...
# ==============================================================================
# استریک روزانه دو مرحله‌ای: ۱. دریافت Grant ID تبلیغاتی ۲. ثبت استریک
# ==============================================================================
...
# ==============================================================================
# اسپین دومرحله‌ای: ۱. دریافت Grant ID تبلیغاتی ۲. چرخش گردونه
# ==============================================================================
        ...
        async with aiohttp.ClientSession(headers=acc_headers, connector=connector) as session:
            while True:
                try:
                    ...
                    config = init_res.get("config", {})
                    spin_cooldown_ms = config.get("spinCooldownHours", 24) * 3600 * 1000

                    spin_cooldown_hours = init_res.get("config", {}).get("spinCooldownHours", 24)
                    spin_cooldown_ms = spin_cooldown_hours * 3600 * 1000

                    streak_cooldown_ms = 24 * 3600 * 1000
                    ...
                    while True:
                        ...
                        # ۱. بررسی استریک روزانه با گرنت دومرحله‌ای (۲۴ ساعت)
                        ...
                        # ۲. بررسی اسپین دو مرحله‌ای بر اساس کانفیگ سرور
...
async def main():
    ...
    print(">>> 2-Stage Streak & Spin Active | Anti-Spam Sleep")
